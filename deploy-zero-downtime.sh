...
# Taypro zero-downtime deployment
#
# Builds in .release-build/ while the live site keeps serving from .next/standalone/.
# During the ~10s PM2 swap, nginx serves the branded 503 maintenance page (not a bare error).
#
# CMS safety matches deploy.sh: production data/cms.sqlite + public/uploads are never
# rsync'd from your laptop. Metrics are asserted before/after swap.
#
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            echo "Usage: ./deploy-zero-downtime.sh [--fast] [--skip-build] [--swap-only] [--checksum]"
            echo ""
            echo "  Zero-downtime deploy: build in staging while the site stays live, then swap."
            ...
            echo ""
            echo "CMS protection (same as deploy.sh):"
            ...
            echo ""
            echo "For full maintenance-window deploy (~15 min 503): ./deploy.sh"
            exit 0
            ;;
    esac
done
...
echo -e "${GREEN}🚀 Zero-downtime deploy (503 only during swap)${NC}"
...
echo -e "${GREEN}✅ Zero-downtime deploy completed!${NC}"
...
echo "  ./deploy-zero-downtime.sh --fast          routine releases"
echo "  ./deploy-zero-downtime.sh --swap-only     re-swap after a failed cutover"
echo "  ./deploy.sh                               full deploy with 503 maintenance window"
echo ""
