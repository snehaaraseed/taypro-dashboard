...
async function main() {
  ...
  // Force page-1 overflow before environmental cards (old bug trigger)
  visuals.PDF_CONTENT_BOTTOM_COVER = 560;
  ...
  buildRoiPdfDocument({
    ...
    inputRows: [
      ...
      { label: `${en.pdfElectricityTariff} (INR/kWh)`, value: "4.5" },
      { label: en.pdfModuleCapacity, value: "550" },
    ],
  });

  const pages = pdf.getNumberOfPages();
  console.log("Pages after forced page-1 overflow:", pages);
  if (pages > 2) {
    console.error("FAIL: expected 2 pages max, got", pages);
    process.exit(1);
  }
  console.log("OK");
}
