...
async function main() {
  ...
  buildRoiPdfDocument({
    ...
    labels: { ...labels, plantName: plantLabel, generatedFor: en.pdfGeneratedFor.replace("{name}", plantLabel) },
    inputRows: inputRows.map((row) =>
      row.label === en.plantName ? { ...row, value: plantLabel } : row
    ),
  });

  console.log("tesadada pages:", pdf.getNumberOfPages());
}
