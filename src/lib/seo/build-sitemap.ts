...
export async function buildSitemapEntries(category?: string): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    const fetchBlogs = !category || category === "blog" || category === "authors";
    const fetchProjects = !category || category === "projects";
    const fetchInsights = !category || category === "insights";
    const fetchPress = !category || category === "press";

    const [blogs, projects, insights, pressReleases] = await Promise.all([
      fetchBlogs ? listPublishedBlogsForSitemap() : Promise.resolve([]),
      fetchProjects ? listPublishedProjectsForSitemap() : Promise.resolve([]),
      fetchInsights ? listPublishedInsightsForSitemap() : Promise.resolve([]),
      fetchPress ? listPublishedPressReleasesForSitemap() : Promise.resolve([]),
    ]);
